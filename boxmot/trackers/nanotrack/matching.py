...
from boxmot.trackers.nanotrack import kalman_filter
...
def fuse_motion(kf, cost_matrix, tracks, detections, only_position=False, lambda_= 0.85):
    if cost_matrix.size == 0:
        return cost_matrix
    gating_dim = 2 if only_position else 4
    gating_threshold = kalman_filter.chi2inv95[gating_dim]
    ...
    for row, track in enumerate(tracks):
        ...
        cost_matrix[row] = lambda_ * cost_matrix[row] + (1 - lambda_) * (gating_distance / gating_threshold)
